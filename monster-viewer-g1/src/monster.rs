...
use crate::{
    address::Addresses,
    hzv::{HitzoneInfoImpl as _, get_hzv_info, get_hzvs, get_hzvs_taikun},
};
        ...
        for hzv_info in get_hzv_info(addresses, monster_struct) {
            ...
            match info.is_real(check1, check2, check3) {
                crate::hzv::HitzoneValidity::Valid => {
                    let hzvs = if monster_id == 0x67 {
                        get_hzvs_taikun(addresses, hzv_info, monster_struct)
                    } else {
                        get_hzvs(addresses, hzv_info, monster_struct)
                    };
                    if let Some(hzvs) = hzvs {
                        if info.scale == 0. {
...
impl MonsterStruct {
    ...
    pub fn part_health(&self, part_idx: usize) -> i16 {
        unsafe { (self.0.wrapping_byte_add(0x358 + part_idx * 8) as *const i16).read() }
    }
    ...
    pub fn current_health(&self, addresses: &Addresses) -> u16 {
        unsafe {
            let health = (self.0.wrapping_byte_add(0x614) as *const u16).read();
    ...
    pub fn max_health(&self, addresses: &Addresses) -> u16 {
        unsafe {
            let health = (self.0.wrapping_byte_add(0x6AC) as *const u16).read();
            let idx = self.struct_idx();
            let key = (addresses.encryption1 as *const u16).read();
            let p = (addresses.encryption2 - idx as usize * 2) as *const u16;
    ...
    pub fn hitzone_check_1(&self) -> u8 {
        unsafe { self.0.wrapping_byte_add(0xC54).read() }
    }

    pub fn hitzone_check_2(&self) -> u8 {
        unsafe { self.0.wrapping_byte_add(0xAA3).read() }
    }

    pub fn hitzone_check_3(&self) -> u16 {
        unsafe { (self.0.wrapping_byte_add(0xB40) as *const u16).read() }
    }
    ...
    fn stun(&self) -> Status {
        unsafe {
            let ptr = self.0;
            let threshold = (ptr.wrapping_byte_add(0xA64) as *const i16).read();
