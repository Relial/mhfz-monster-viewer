...
fn hook_damage_calc<'a>(
    ...
    let on_call = move |reg: *mut Registers| unsafe {
        if let Some(own_player) = addresses.own_player_addr() {
            let damage_source_addr = (((*reg).ebp - 0x7C) as *const usize).read();
            if damage_source_addr == own_player {
                let hitzone_info = ((*reg).ecx as *const HitzoneInfo).read();
                let damaged_monster = MonsterStruct::new((*reg).eax as *const u8);
